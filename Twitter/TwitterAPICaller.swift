...
class TwitterAPICaller: BDBOAuth1SessionManager {    
    ...
    func postTweet(tweetText:String, success: @escaping () -> (), failure: @escaping (Error) -> ()){
        ...
        TwitterAPICaller.client?.post(baseURL, parameters: ["status":tweetText], progress: nil, success: { (task: URLSessionDataTask, response: Any?) in
            success()
        }, failure: { (task: URLSessionDataTask?, error: Error) in
            failure(error)
        })
    ...
    func favoriteTweet(tweetId:Int, success: @escaping () -> (), failure: @escaping (Error) -> ()){
        let baseURL = "https://api.twitter.com/1.1/favorites/create.json"
        TwitterAPICaller.client?.post(baseURL, parameters: ["id":tweetId], progress: nil, success: { (task: URLSessionDataTask, response: Any?) in
            success()
        }, failure: { (task: URLSessionDataTask?, error: Error) in
            failure(error)
        })
    ...
    func unfavoriteTweet(tweetId:Int, success: @escaping () -> (), failure: @escaping (Error) -> ()){
        let baseURL = "https://api.twitter.com/1.1/favorites/destroy.json"
        TwitterAPICaller.client?.post(baseURL, parameters: ["id":tweetId], progress: nil, success: { (task: URLSessionDataTask, response: Any?) in
            success()
        }, failure: { (task: URLSessionDataTask?, error: Error) in
            failure(error)
    ...
    func retweet(tweetId:Int, success: @escaping () -> (), failure: @escaping (Error) -> ()){
        let baseURL = "https://api.twitter.com/1.1/statuses/retweet/\(tweetId).json"
        TwitterAPICaller.client?.post(baseURL, parameters: ["id":tweetId], progress: nil, success: { (task: URLSessionDataTask, response: Any?) in
            success()
        }, failure: { (task: URLSessionDataTask?, error: Error) in
            failure(error)
    ...
    func unretweet(tweetId:Int, success: @escaping () -> (), failure: @escaping (Error) -> ()){
        let baseURL = "https://api.twitter.com/1.1/statuses/unretweet/\(tweetId).json"
        TwitterAPICaller.client?.post(baseURL, parameters: ["id":tweetId], progress: nil, success: { (task: URLSessionDataTask, response: Any?) in
            success()
        }, failure: { (task: URLSessionDataTask?, error: Error) in
            failure(error)
